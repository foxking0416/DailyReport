...
namespace HuaChun_DailyReport
{
    public partial class ProjectIncreaseForm : Form
    {
        ...
        protected void InsertIntoDB()
        {
            ...
            commandStr = commandStr + Functions.TransferDateTimeToSQL(uiDateTimeBid.Value)  + "','";//13
            commandStr = commandStr + Functions.TransferDateTimeToSQL(uiDateTimeStart.Value) + "','";//14
        ...
        protected void CalculateByDuration()
        {
            ...
            DateTime FinishDate = dayCompute.CountByDuration(uiDateTimeStart.Value, Convert.ToSingle(uiNumericDuration.Value));
            uiDateTimeFinish.Value = FinishDate;
            uiNumericDays.Value = FinishDate.Subtract(uiDateTimeStart.Value).Days + 1;
        }
        ...
        private void BtnLoadFromTendor_Click( object sender, EventArgs e )
        {
            LoadProjectInfoFromTendorForm TendorSearchForm = new LoadProjectInfoFromTendorForm( this, m_Sql );
            TendorSearchForm.ShowDialog();
        }
        ...
        private void RadioButton123_CheckedChanged(object sender, EventArgs e)
        {
            if (uiRadioBtnRestrictSchedule.Checked == true)//限期完工
            {
                ...
                int duration = uiDateTimeFinish.Value.Date.Subtract(uiDateTimeStart.Value.Date).Days;
        ...
        {
            int iDuration = uiDateTimeFinish.Value.Date.Subtract(uiDateTimeStart.Value.Date).Days;

            if (iDuration < 0)
            {
                MessageBox.Show("完工日期不得早於開工日期", "錯誤", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                uiDateTimeFinish.Value = uiDateTimeStart.Value;
            }
            else
            {
                if (uiRadioBtnRestrictSchedule.Checked == true)
